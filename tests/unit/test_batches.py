...
def test_allocating_to_batch_reduce_the_available_quantity():
...
def test_can_allocate_if_available_equal_to_requred():
    batch, line = make_batch_and_line("ELEGANT-LAMP", 2, 2)
    assert batch.can_allocate(line)


def test_cannot_allocate_if_skus_do_not_not_match():
    batch = Batch("batch-001", "UNCONFORTABLE-CHAIR", 100, eta=None)
    different_sku_line = OrderLine("order-123", "EXPENSIVE-TOASTER", 10)
    assert batch.can_allocate(different_sku_line) is False


def test_allocation_is_idepotent():
